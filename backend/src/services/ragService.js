...
export const performQuery = async (userId, subject, queryText, topK) => {
  try {
    log.debug(
      `Querying RAG - user: ${userId}, subject: ${subject}, query: ${queryText}`,
    );
    ...
    log.success(
      `RAG query successful, ${response.data.results?.length || 0} results returned`,
    );
...
export const getServiceStats = async () => {
  try {
    const response = await ragAxios.get("/api/v1/ingest/stats", {
      timeout: 5000,
    });
    return response.data;
  } catch (error) {
