...
const analyzeTopics = (ragResults) => {
  ...
  for (const result of ragResults) {
    ...
    for (const paper of papers) {
      ...
      const estimatedMarks =
        difficulty === "hard" ? 8 : difficulty === "easy" ? 3 : 5;
  ...
  for (const [topic, data] of topicMap) {
    ...
    data.relevanceScore =
      data.frequency > 0 ? data.relevanceScore / data.frequency : 0;
...
export const generatePlan = async (userId, assessmentData) => {
  try {
    ...
    const queryText =
      topicsList.length > 0 ? `${subject}: ${topicsList.join(", ")}` : subject;

    log.info(
      `Querying RAG for subject: ${subject}, topics: ${topicsList.join(", ")}`,
    );
...
7. Make the plan realistic and actionable`;
    ...
    log.info(
      `AI plan parsed: ${aiPlan.prioritizedTopics?.length || 0} topics, ${aiPlan.weeklySchedule?.length || 0} weeks`,
    );
    ...
    log.success(
      `AI plan generated: ${enhancedPlan.prioritizedTopics?.length || 0} topics`,
    );
...
const analyzeRAGResults = (ragResults) => {
  ...
  const summaryLines = [
    ...
    topics.length > 0
      ? `Top topics by frequency: ${topics
          .slice(0, 5)
          .map((t) => `${t.name} (${t.frequency}x)`)
          .join(", ")}`
      : "",
  ].filter(Boolean);
...
const generateStrategyTips = (topicData, understandingGap) => {
  ...
  if (topicData.yearCount && topicData.yearCount > 3) {
    tips.push(
      `Consistent topic - appeared in ${topicData.yearCount} different years`,
    );
  }
...
const generateRecommendations = (gapVector, topics) => {
  ...
  if (gapVector.overallConfidence < 2.5) {
    recommendations.push({
      type: "critical",
      message:
        "Focus on understanding core concepts before attempting advanced problems",
    });
  }
  ...
  if (highPriorityCount > 10) {
    recommendations.push({
      type: "warning",
      message:
        "Many high-priority topics identified. Consider extending study time or prioritizing ruthlessly.",
    });
  }
